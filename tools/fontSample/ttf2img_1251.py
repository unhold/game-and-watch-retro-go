...
def Paint_Fontogn(font_name, priname, font_size:int, out_size:int, xoffset:int, yoffset:int):
    ...
    for x in range(16):
        for y in range(16):
            ...
            #if (chrno > 127):
            try:
                character = chrno.decode('cp1251')
                #print(character)
            except:
                #print(y*16+x)
                character = bytes([y*16+x])
                #print(character)
...
def process_onefile(filename, fontdef):
    ...
    Paint_Fontogn(filename, fdef["_a_"] + "", int(fdef["fontsize"]), int(fdef["fixedsize"]), int(fdef["xoffset"]), int(fdef["yoffset"]))  #ogn font image
