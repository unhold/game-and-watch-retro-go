...
def Txt_Fromimg(font_name, priname, out_size:int, ckvale:int):
    ...
    if (priname == "__") :
        priname = ""
    py_file = "fcdata.py"
    txt_file = priname + str((Path(font_name)).stem + ".txt")
    out_file = priname + str((Path(font_name)).stem + ".h")
...
def process_onefile(filename, fontdef):
    ...
    fdef.setdefault("_a_", "")
    Txt_Fromimg(filename, fdef["_a_"] + "", int(fdef["fixedsize"]), int(fdef["check"]))  #ogn font image
